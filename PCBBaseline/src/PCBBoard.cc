...
namespace PCBDesign {
    ...
    void PCBBoard::display() const {
        ...
        void PCBBoard::runRouting(const RoutingRules& rules) {
            ...
            // Set the net ordering strategy (default, ML based and GNN based)

            //Uncomment to apply default net ordering
            router->setNetOrderingStrategy(std::make_unique<DefaultNetOrdering>());
            //Uncomment to apply ML based net ordering

            //router->setNetOrderingStrategy(std::make_unique<MLGANetOrdering>("src/ga_optimizer.py", "data/ACtoDCconverter.kicad_pcb"));
            
            //Uncomment to apply GNN based net ordering

            // router->setNetOrderingStrategy(std::make_unique<GNNNetOrdering>(
            //    "src/gnn_optimizer.py",                   // path to your optimizer script
