...
class InputHandler extends Lifecycle_1.Disposable {
    ...
    constructor(_bufferService, _charsetService, _coreService, _logService, _optionsService, _oscLinkService, _coreMouseService, _unicodeService, _parser = new EscapeSequenceParser_1.EscapeSequenceParser()) {
        ...
        this._specialColors = [Types_1.SpecialColorIndex.FOREGROUND, Types_1.SpecialColorIndex.BACKGROUND, Types_1.SpecialColorIndex.CURSOR];
    ...
    selectProtected(params) {
        ...
        if (p === 1)
            this._curAttrData.bg |= Constants_1.BgFlags.PROTECTED;
        if (p === 2 || p === 0)
            this._curAttrData.bg &= ~Constants_1.BgFlags.PROTECTED;
        return true;
    }
    ...
    requestMode(params, ansi) {
        ...
        const b2v = (value) => value ? 1 /* V.SET */ : 2 /* V.RESET */;
        const p = params.params[0];
        if (ansi) {
            if (p === 2)
                return f(p, 4 /* V.PERMANENTLY_RESET */);
            if (p === 4)
                return f(p, b2v(cs.modes.insertMode));
            if (p === 12)
                return f(p, 3 /* V.PERMANENTLY_SET */);
            if (p === 20)
                return f(p, b2v(opts.convertEol));
            return f(p, 0 /* V.NOT_RECOGNIZED */);
        }
        if (p === 1)
            return f(p, b2v(dm.applicationCursorKeys));
        if (p === 3)
            return f(p, opts.windowOptions.setWinLines ? (cols === 80 ? 2 /* V.RESET */ : cols === 132 ? 1 /* V.SET */ : 0 /* V.NOT_RECOGNIZED */) : 0 /* V.NOT_RECOGNIZED */);
        if (p === 6)
        ...
        if (p === 1048)
            return f(p, 1 /* V.SET */); // xterm always returns SET here
    ...
    _updateAttrColor(color, mode, c1, c2, c3) {
        if (mode === 2) {
            color |= Constants_1.Attributes.CM_RGB;
            color &= ~Constants_1.Attributes.RGB_MASK;
            color |= AttributeData_1.AttributeData.fromColorRGB([c1, c2, c3]);
        }
        else if (mode === 5) {
            color &= ~(Constants_1.Attributes.CM_MASK | Constants_1.Attributes.PCOLOR_MASK);
            color |= Constants_1.Attributes.CM_P256 | (c1 & 0xff);
        }
        return color;
    }
    /**
    ...
    _processUnderline(style, attr) {
        ...
        attr.fg |= Constants_1.FgFlags.UNDERLINE;
    ...
    _processSGR0(attr) {
        ...
        attr.extended.underlineStyle = Constants_1.UnderlineStyle.NONE;
        attr.extended.underlineColor &= ~(Constants_1.Attributes.CM_MASK | Constants_1.Attributes.RGB_MASK);
        attr.updateExtended();
    }
    /**
    ...
    charAttributes(params) {
        ...
        for (let i = 0; i < l; i++) {
            ...
            if (p >= 30 && p <= 37) {
                // fg color 8
                attr.fg &= ~(Constants_1.Attributes.CM_MASK | Constants_1.Attributes.PCOLOR_MASK);
                attr.fg |= Constants_1.Attributes.CM_P16 | (p - 30);
            }
            else if (p >= 40 && p <= 47) {
                // bg color 8
                attr.bg &= ~(Constants_1.Attributes.CM_MASK | Constants_1.Attributes.PCOLOR_MASK);
                attr.bg |= Constants_1.Attributes.CM_P16 | (p - 40);
            }
            else if (p >= 90 && p <= 97) {
                // fg color 16
                attr.fg &= ~(Constants_1.Attributes.CM_MASK | Constants_1.Attributes.PCOLOR_MASK);
                attr.fg |= Constants_1.Attributes.CM_P16 | (p - 90) | 8;
            }
            else if (p >= 100 && p <= 107) {
                // bg color 16
                attr.bg &= ~(Constants_1.Attributes.CM_MASK | Constants_1.Attributes.PCOLOR_MASK);
                attr.bg |= Constants_1.Attributes.CM_P16 | (p - 100) | 8;
            }
            else if (p === 0) {
                // default
                ...
                // bold text
                attr.fg |= Constants_1.FgFlags.BOLD;
            }
            else if (p === 3) {
                // italic text
                attr.bg |= Constants_1.BgFlags.ITALIC;
            }
            else if (p === 4) {
                // underlined text
                attr.fg |= Constants_1.FgFlags.UNDERLINE;
                this._processUnderline(params.hasSubParams(i) ? params.getSubParams(i)[0] : Constants_1.UnderlineStyle.SINGLE, attr);
            }
            else if (p === 5) {
                // blink
                attr.fg |= Constants_1.FgFlags.BLINK;
            }
            else if (p === 7) {
                // inverse and positive
                // test with: echo -e '\e[31m\e[42mhello\e[7mworld\e[27mhi\e[m'
                attr.fg |= Constants_1.FgFlags.INVERSE;
            }
            else if (p === 8) {
                // invisible
                attr.fg |= Constants_1.FgFlags.INVISIBLE;
            }
            else if (p === 9) {
                // strikethrough
                attr.fg |= Constants_1.FgFlags.STRIKETHROUGH;
            }
            else if (p === 2) {
                // dimmed text
                attr.bg |= Constants_1.BgFlags.DIM;
            }
            else if (p === 21) {
                // double underline
                this._processUnderline(Constants_1.UnderlineStyle.DOUBLE, attr);
            }
            else if (p === 22) {
                // not bold nor faint
                attr.fg &= ~Constants_1.FgFlags.BOLD;
                ...
                // not underlined
                attr.fg &= ~Constants_1.FgFlags.UNDERLINE;
                this._processUnderline(Constants_1.UnderlineStyle.NONE, attr);
            }
            else if (p === 25) {
                // not blink
                attr.fg &= ~Constants_1.FgFlags.BLINK;
                ...
                // reset fg
                attr.fg &= ~(Constants_1.Attributes.CM_MASK | Constants_1.Attributes.RGB_MASK);
                attr.fg |= BufferLine_1.DEFAULT_ATTR_DATA.fg & (Constants_1.Attributes.PCOLOR_MASK | Constants_1.Attributes.RGB_MASK);
            }
            else if (p === 49) {
                // reset bg
                attr.bg &= ~(Constants_1.Attributes.CM_MASK | Constants_1.Attributes.RGB_MASK);
                attr.bg |= BufferLine_1.DEFAULT_ATTR_DATA.bg & (Constants_1.Attributes.PCOLOR_MASK | Constants_1.Attributes.RGB_MASK);
            }
                ...
                // overline
                attr.bg |= Constants_1.BgFlags.OVERLINE;
            }
            else if (p === 55) {
                // not overline
                attr.bg &= ~Constants_1.BgFlags.OVERLINE;
        ...
        while (slots.length > 1) {
            ...
            if (/^\d+$/.exec(idx)) {
                ...
                if (isValidColorIndex(index)) {
                    if (spec === '?') {
                        event.push({ type: Types_1.ColorRequestType.REPORT, index });
                    }
                    else {
                        const color = (0, XParseColor_1.parseColor)(spec);
                        if (color) {
                            event.push({ type: Types_1.ColorRequestType.SET, index, color });
                        }
                    }
                }
            }
    ...
    _setOrReportSpecialColor(data, offset) {
        ...
        for (let i = 0; i < slots.length; ++i, ++offset) {
            if (offset >= this._specialColors.length)
                break;
            if (slots[i] === '?') {
                this._onColor.fire([{ type: Types_1.ColorRequestType.REPORT, index: this._specialColors[offset] }]);
            }
            else {
                const color = (0, XParseColor_1.parseColor)(slots[i]);
                if (color) {
                    this._onColor.fire([{ type: Types_1.ColorRequestType.SET, index: this._specialColors[offset], color }]);
                }
            }
        }
        return true;
    ...
    restoreIndexedColor(data) {
        if (!data) {
            this._onColor.fire([{ type: Types_1.ColorRequestType.RESTORE }]);
            return true;
        }
        const event = [];
        const slots = data.split(';');
        for (let i = 0; i < slots.length; ++i) {
            if (/^\d+$/.exec(slots[i])) {
                ...
                if (isValidColorIndex(index)) {
                    event.push({ type: Types_1.ColorRequestType.RESTORE, index });
                }
            }
        }
        if (event.length) {
     ...
     */
    restoreFgColor(data) {
        this._onColor.fire([{ type: Types_1.ColorRequestType.RESTORE, index: Types_1.SpecialColorIndex.FOREGROUND }]);
        return true;
    }
    /**
     ...
     */
    restoreBgColor(data) {
        this._onColor.fire([{ type: Types_1.ColorRequestType.RESTORE, index: Types_1.SpecialColorIndex.BACKGROUND }]);
        return true;
    }
    /**
    ...
    restoreCursorColor(data) {
        this._onColor.fire([{ type: Types_1.ColorRequestType.RESTORE, index: Types_1.SpecialColorIndex.CURSOR }]);
        return true;
    }
    /**
     * ESC E
    ...
    selectDefaultCharset() {
        this._charsetService.setgLevel(0);
        this._charsetService.setgCharset(0, Charsets_1.DEFAULT_CHARSET); // US (default)
        return true;
    }
    /**
     * ESC ( C
     ...
     */
    _eraseAttrData() {
        this._eraseAttrDataInternal.bg &= ~(Constants_1.Attributes.CM_MASK | 0xFFFFFF);
    ...
    screenAlignmentPattern() {
        ...
        cell.content = 1 << Constants_1.Content.WIDTH_SHIFT | 'E'.charCodeAt(0);
