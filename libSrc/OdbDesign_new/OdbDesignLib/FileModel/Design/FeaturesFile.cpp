...
namespace Odb::Lib::FileModel::Design
{
	FeaturesFile::FeaturesFile()
		: m_units(""), m_path(""), m_directory(""), m_numFeatures(0), m_id((unsigned)-1)
	{
	}
		...
		try
		{
			...
			while (std::getline(featuresFile, line))
			{
				...
				if (!line.empty())
				{
					...
					{
						// Skip pure index lines (like "&2")
						if (line.size() <= 3 && line.find_first_not_of("&0123456789") == std::string::npos)
						{
							continue;
						}

						// Extract everything after &number (space-safe)
						size_t firstSpace = line.find(' ');
						if (firstSpace != std::string::npos)
						{
							std::string content = line.substr(firstSpace + 1);
							m_attributeTextValues.push_back(content);
						}
						// Optional: Handle malformed lines with only &number
						// else {
						//     m_attributeTextValues.push_back("");
						//     // or: throw_parse_error(...)
						// }
					}
					// else if (line.find(ATTRIBUTE_VALUE_TOKEN) == 0)
					// {
					...
					else if (line.find(FeatureRecord::PAD_TOKEN) == 0)
					{
						...
						// Only read resize factor if symbol num is -1
						if (pFeatureRecord->apt_def_symbol_num == -1)
						{
							if (!(lineStream >> pFeatureRecord->apt_def_resize_factor))
							{
						...
						// Read orientation definition
						if (!(lineStream >> pFeatureRecord->orient_def))
						{
							throw_parse_error(m_path, line, token, lineNumber);
						}

						// Check if there's more data before the semicolon (orientation rotation)
						std::string nextToken;
						lineStream >> nextToken;

						// If next token is not the start of attributes (;)
						if (nextToken.find(';') == std::string::npos)
						{
							// Try to parse as orientation rotation
							try
							{
								pFeatureRecord->orient_def_rotation = std::stod(nextToken);
								// Now read the attributes
								lineStream >> nextToken;
							}
							catch (...)
							{
								// Not a number, treat as part of attributes
								nextToken = "";
							}
						}
						...
						if (!nextToken.empty())
						{
							if (!pFeatureRecord->ParseAttributeLookupTable(nextToken))
							{
								throw_parse_error(m_path, line, token, lineNumber);
							}
						}
	...
	const std::vector<std::string> &FeaturesFile::GetAttributeNames() const
	{
		return m_attributeNames;
	}
	const std::vector<std::string> &FeaturesFile::GetAttributeTextValues() const
	{
