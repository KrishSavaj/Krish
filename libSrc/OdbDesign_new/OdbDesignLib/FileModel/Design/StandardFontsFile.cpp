...
namespace Odb::Lib::FileModel::Design
{
    ...
    // Getter for X size
    float StandardFontsFile::GetXSize() const
    {
        return m_xSize;
    }

    // Getter for Y size
    float StandardFontsFile::GetYSize() const
    {
        return m_ySize;
    }

    // Getter for offset
    float StandardFontsFile::GetOffset() const
    {
        return m_offset;
    }

    // Getter for character blocks
    const StandardFontsFile::CharacterBlock::Vector &StandardFontsFile::GetCharacterBlocks() const
    {
        return m_characterBlocks;
    }
            ...
            while (std::getline(standardFile, line))
            {
                ...
                if (!line.empty())
                {
                    ...
                    else if (line.find(CharacterBlock::LineRecord::RECORD_TOKEN) == 0)
                    {
                        ...
                        {
                        case 'P':
                            pLineRecord->polarity = Polarity::POSITIVE;
                            break;
                        case 'N':
                            pLineRecord->polarity = Polarity::NEGATIVE;
                            break;
                        default:
                            throw_parse_error(m_path, line, token, lineNumber);
                        }
                        ...
                        {
                        case 'R':
                            pLineRecord->shape = LineShape::Round;
                            break;
                        case 'S':
                            pLineRecord->shape = LineShape::Square;
                            break;
                        default:
                            throw_parse_error(m_path, line, token, lineNumber);
                        }
