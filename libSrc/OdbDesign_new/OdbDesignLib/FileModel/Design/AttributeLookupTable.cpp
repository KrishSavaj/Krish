...
namespace Odb::Lib::FileModel::Design
{
	...
	bool AttributeLookupTable::ParseAttributeLookupTable(const std::string &attributeLookupTableString)
	{
		...
		while (std::getline(ss, token, ';'))
		{
			...
			if (eqPos != std::string::npos)
			{
				...
				m_attributeLookupTable[key] = val;
			}
			else
			{
				// Just a key without value, like '29'
				m_attributeLookupTable[token] = "";
			}
		}
	...
	// 				{
	// 					if (!std::getline(aa_ss, name, '='))
	// 						return false;
	// 					if (!std::getline(aa_ss, value))
	// 						return false;
	// 				}
	// 				else
	// 				{
	...
	// 		{
	// 			if (!std::getline(token_ss, name, '='))
	// 				return false;
	// 			if (!std::getline(token_ss, value))
	// 				return false;
	// 		}
