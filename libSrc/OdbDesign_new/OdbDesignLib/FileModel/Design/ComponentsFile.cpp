...
namespace Odb::Lib::FileModel::Design
{
	ComponentsFile::ComponentsFile()
		: m_id((unsigned int)-1), m_side(BoardSide::BsNone)
	{
	}
	...
	int ComponentsFile::GetId() const
	{
		return m_id;
	}
	...
	bool ComponentsFile::Parse(std::filesystem::path directory)
	{
		...
		try
		{
			...
			{
				m_side = m_layerName == TOP_COMPONENTS_LAYER_NAME ? BoardSide::Top : BoardSide::Bottom;
			}
			else
			{
			...
			while (std::getline(componentsFile, line))
			{
				...
				if (!line.empty())
				{
					...
					else if (line.find(PropertyRecord::RECORD_TOKEN) == 0)
					{
						...
						{
							throw_parse_error(m_path, line, "Missing opening quote", lineNumber);
						}

						// Find the closing single quote
						size_t endQuotePos = line.rfind('\'');
						if (endQuotePos == std::string::npos || endQuotePos <= quotePos)
						{
							throw_parse_error(m_path, line, "Missing closing quote", lineNumber);
						}

						// Extract the value between quotes
						pPropertyRecord->value = line.substr(quotePos + 1, endQuotePos - quotePos - 1);

						// Parse any float values that might follow the quoted string
						std::string remaining = line.substr(endQuotePos + 1);
						std::stringstream remainingStream(remaining);
						float f;
						while (remainingStream >> f)
						{
							pPropertyRecord->floatValues.push_back(f);
						}
					...
					else if (line.find(ComponentsFile::BomDescriptionRecord::CPN_RECORD_TOKEN) == 0)
					{
						...
						// Read the rest of the line as CPN value
						std::getline(lineStream, pCurrentBomDescriptionRecord->cpn);
						Utils::str_trim(pCurrentBomDescriptionRecord->cpn);

						if (pCurrentBomDescriptionRecord->cpn.empty())
						{
							logdebug("Empty CPN value found at line " + std::to_string(lineNumber));
						}
					}
					else if (line.find(ComponentsFile::BomDescriptionRecord::PKG_RECORD_TOKEN) == 0)
					{
						std::string token;
						lineStream >> token; // Read "PKG"

						if (pCurrentBomDescriptionRecord == nullptr)
						{
							logerror("PKG record found without preceding CPN at line " + std::to_string(lineNumber));
							continue;
						}

						std::getline(lineStream, pCurrentBomDescriptionRecord->pkg);
						Utils::str_trim(pCurrentBomDescriptionRecord->pkg);

						if (pCurrentBomDescriptionRecord->pkg.empty())
						{
							logdebug("Empty PKG value found at line " + std::to_string(lineNumber));
						}
					}
					else if (line.find(ComponentsFile::BomDescriptionRecord::IPN_RECORD_TOKEN) == 0)
					{
						std::string token;
						lineStream >> token; // Read "IPN"

						if (pCurrentBomDescriptionRecord == nullptr)
						{
							logerror("IPN record found without preceding CPN at line " + std::to_string(lineNumber));
							continue;
						}

						std::getline(lineStream, pCurrentBomDescriptionRecord->ipn);
						Utils::str_trim(pCurrentBomDescriptionRecord->ipn);
					}
					else if (line.find(ComponentsFile::BomDescriptionRecord::DSC_RECORD_TOKEN) == 0)
					{
						std::string token;
						lineStream >> token; // Read "DSC"

						if (pCurrentBomDescriptionRecord == nullptr)
						{
							logerror("DSC record found without preceding CPN at line " + std::to_string(lineNumber));
							continue;
						}

						std::string description;
						std::getline(lineStream, description);
						Utils::str_trim(description);
					...
					{
						std::string token;
						lineStream >> token; // Read "VPL_VND"

						if (pCurrentBomDescriptionRecord == nullptr)
						{
							logerror("VPL_VND record found without preceding CPN at line " + std::to_string(lineNumber));
							continue;
						}

						std::getline(lineStream, pCurrentBomDescriptionRecord->vpl_vnd);
						Utils::str_trim(pCurrentBomDescriptionRecord->vpl_vnd);
					}
					else if (line.find(ComponentsFile::BomDescriptionRecord::VPL_MPN_RECORD_TOKEN) == 0)
					{
						std::string token;
						lineStream >> token; // Read "VPL_MPN"

						if (pCurrentBomDescriptionRecord == nullptr)
						{
							logerror("VPL_MPN record found without preceding CPN at line " + std::to_string(lineNumber));
							continue;
						}

						std::getline(lineStream, pCurrentBomDescriptionRecord->vpl_mpn);
						Utils::str_trim(pCurrentBomDescriptionRecord->vpl_mpn);
					}
					else if (line.find(ComponentsFile::BomDescriptionRecord::VND_RECORD_TOKEN) == 0)
					{
						std::string token;
						lineStream >> token; // Read "VND"

						if (pCurrentBomDescriptionRecord == nullptr)
						{
							logerror("VND record found without preceding CPN at line " + std::to_string(lineNumber));
							continue;
						}

						std::getline(lineStream, pCurrentBomDescriptionRecord->vnd);
						Utils::str_trim(pCurrentBomDescriptionRecord->vnd);
					}
					else if (line.find(ComponentsFile::BomDescriptionRecord::MPN_RECORD_TOKEN) == 0)
					{
						std::string token;
						lineStream >> token; // Read "MPN"

						if (pCurrentBomDescriptionRecord == nullptr)
						{
							logerror("MPN record found without preceding CPN at line " + std::to_string(lineNumber));
							continue;
						}

						std::getline(lineStream, pCurrentBomDescriptionRecord->mpn);
						Utils::str_trim(pCurrentBomDescriptionRecord->mpn);

						// MPN is typically the last record, so store the completed record
						if (!pCurrentBomDescriptionRecord->cpn.empty())
						{
							m_bomDescriptionRecordsByCpn[pCurrentBomDescriptionRecord->cpn] = pCurrentBomDescriptionRecord;
							pCurrentBomDescriptionRecord.reset();
